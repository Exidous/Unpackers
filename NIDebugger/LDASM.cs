...
namespace LDASM_Sharp
{
    public class LDASM
    {

        const byte OP_NONE = 0x00;
        const byte OP_INVALID = 0x80;

        const byte OP_DATA_I8 = 0x01;
        const byte OP_DATA_I16 = 0x02;
        const byte OP_DATA_I16_I32 = 0x04;
        const byte OP_DATA_I16_I32_I64 = 0x08;
        const byte OP_EXTENDED = 0x10;
        const byte OP_RELATIVE = 0x20;
        const byte OP_MODRM = 0x40;
        const byte OP_PREFIX = 0x80;

        const byte F_INVALID = 0x01;
        const byte F_PREFIX = 0x02;
        const byte F_REX = 0x04;
        const byte F_MODRM = 0x08;
        const byte F_SIB = 0x10;
        const byte F_DISP = 0x20;
        const byte F_IMM = 0x40;
        const byte F_RELATIVE = 0x80;

        #region opcodeFlags
        byte[] OpcodeFlags = new byte[256] 
        {
            /* 00 */        OP_MODRM,
            /* 01 */        OP_MODRM,
            /* 02 */        OP_MODRM,
        ...
        byte[] OpcodeFlagsExt = new byte[256]
        {
            /* 0F00 */      OP_MODRM,
            /* 0F01 */      OP_MODRM,
            /* 0F02 */      OP_MODRM,
        ...
        private byte cflags(byte op)
        {
            return OpcodeFlags[op];
        }

        private byte cflags_ex(byte op)
        {
            return OpcodeFlagsExt[op];
        }
    ...
    public struct ldasm_data
    {
        ...
        public byte modrm;
        public byte sib;
        public int opcd_offset;
        public byte opcd_size;
        public int disp_offset;
        public byte disp_size;
        public int imm_offset;
